...
if(file_exists($lock_file)) {
	showheader();
	showerror('升级被锁定，说明您已经进行过升级操作，如需继续升级请删除 '.str_replace(UC_ROOT, '', $lock_file).' 文件，再尝试本操作');
	showfooter();
}
...
<?php
	...
	if($total) {
		...
		@touch($lock_file);
		echo "<font style='font-size:20px;'>升级完成</font>。<br>原短消息表(".UC_DBTABLEPRE."pms表)已经不再使用，建议您在测试短消息功能及数据没有问题之后，删除此表。<br>将advanced/uc_client中的文件覆盖您的uc_client客户端。";
	}
