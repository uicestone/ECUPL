		...
		if ( $rs = $this->checkMelonCondition( $user, $stock_id ) )
		{
			...
			if ( $_G['gp_cuttype'] == '1' )
			{
				...
				else
				{
					...
					DB::query("INSERT INTO ".DB::table('kfsm_smlog')." (type, username2, field, descrip, timestamp, ip) VALUES('分红管理', '{$user[username]}', '股票分红', '上市公司 {$rs[stockname]} 每 100 股现金派息 ".number_format($cutnum1,2)." 元', '$_G[timestamp]', '$_G[clientip]')");
