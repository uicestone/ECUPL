...
$baseScript = 'action=plugins&operation=config&do='.$pluginid.'&identifier=stock&pmod=admincp';
...
switch ( $mod )
{
	...
	case 'about':
		...
		include template('stock:m_about');
	break;
	case 'stockset':
		...
		if ( empty($section) )
		{
			...
			include template('stock:m_stockset');
		}
		else if ( $section == 'editstock' )
		{
			if ( empty($step) )
			{
				...
				include template('stock:m_editstock');
			}
			else if ( $step == '2' )
			{
				$stock->updateStock();
		...
		else if ( $section == 'delstock' )
		{
			if ( empty($step) )
			{
				...
				include template('stock:m_delstock');
			}
			else if ( $step == '2' )
			{
	...
	case 'userset':
		...
		if ( empty($section) )
		{
			...
			include template('stock:m_userset');
		}
		else if ( $section == 'edituser' )
		{
			if ( empty($step) )
			{
				...
				include template('stock:m_edituser');
			}
			else if ( $step == '2' )
			{
				$user->updateUser();
		...
		else if ( $section == 'deluser' )
		{
			if ( empty($step) )
			{
				...
				include template('stock:m_deluser');
			}
			else if ( $step == '2' )
			{
				$user->exeDeleteUser();
	...
	case 'trusts':
		...
		if ( empty($section) )
		{
			...
			include template('stock:m_trust');
		}
	...
	case 'esset':
		...
		include template('stock:m_applyset');
	break;
	case 'usmng':
		require_once 'require/acp_apply.php';
		$apply = new Apply;
	...
	case 'tools':
		...
		if ( empty($section) )
		{
			...
			include template('stock:m_tools');
		}
